...
def main():
    ...
    entity_list = mem.read_mem(base + helper.Pointer.entity_list, c_int64())

    while not window_should_close():
        matrix = mem.read_array(base + helper.Pointer.view_matrix, c_float, 16)
        local_player_addr = mem.read_mem(base + helper.Pointer.local_player, c_int64())
        ...
        for ent_addr in ent_array:
            if ent_addr != 0 and ent_addr != local_player_addr:
                try:
                    ent = Entity()
                    ent.data = mem.read_mem(ent_addr, helper.Entity(), False)
                    if ent.data.health <= 0:
                        continue
                    ent.pos2d = helper.world_to_screen(matrix, ent.data.pos)
                    ent.color = RED if ent.data.team == 0 else BLUE
                    ent.draw_snapline()
                    ent.draw_name()
                except IOError:
                    continue
